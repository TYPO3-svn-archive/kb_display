 ...
 */
class FrontendPlugin extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	var $prefixId      = 'tx_kbdisplay_pi_cached';												// Same as class name
	var $scriptRelPath = 'Classes/FrontendPlugin.php';		// Path to this script relative to the extension dir.
		...
		if (is_array($hooks = $this->hookConfig[$name])) {
			foreach ($hooks as $hookKey => $hookConfig) {
				...
				if ($hookConfig['object'] && ($method = $hookConfig['method'])) {
					$object = GeneralUtility::getUserObj($hookConfig['object']);
					if (is_object($object) && method_exists($object, $method)) {
						$object->$method($params, $this);
					}
				} elseif ($hookConfig['userFunc']) {
					GeneralUtility::callUserFunction($hookConfig['userFunc'], $params, $this);
				}
			}
		}
	}
	...
	function getContent($disableQuery) {
		...
		//
		// TODO: Usage of "queryController"
		// The query controller should perform the required steps on his own. Currently
		// this class itself rather acts like a controller
	...
	public function assignToSmarty() {
		$this->smarty->assign('TYPO3_SITE_URL', GeneralUtility::getIndpEnv('TYPO3_SITE_URL'));
	...
	function initObject_queryController() {
		$this->queryController = GeneralUtility::makeInstance('thinkopen_at\kbDisplay\Query\Controller');
	 ...
	 */
	function loadParams() {
		$this->showUids = GeneralUtility::intExplode(',', $this->piVars['view']);
		...
		if (is_array($this->config['showUid.'])) {
			...
			$this->showUids = GeneralUtility::intExplode(',', $uidResult);
		}
		$doShow = false;
		if (!$this->showUid) {
